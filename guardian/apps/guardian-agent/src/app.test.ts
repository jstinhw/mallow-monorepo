...
describe("POST /check", () => {
  ...
  it("rejects a malformed body before starting the stream", async () => {
    const app = buildServer(fakeAnalyze)
    const res = await app.inject({ method: "POST", url: "/check", payload: { ...VALID_BODY, to: "not-an-address" } })
  ...
  it("rejects an unsupported chain", async () => {
    const app = buildServer(fakeAnalyze)
    const res = await app.inject({ method: "POST", url: "/check", payload: { ...VALID_BODY, chainId: 999999 } })
