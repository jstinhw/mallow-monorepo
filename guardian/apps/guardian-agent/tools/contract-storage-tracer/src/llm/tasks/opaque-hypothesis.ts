...
function userPrompt(routes: readonly OpaqueRouteInput[]): string {
  return routes
    .map((r) => `Route ${r.contract}.${r.functionName} — ${r.note}\nSource (truncated):\n${r.sourceSlice.slice(0, 2500)}`)
    .join("\n\n---\n\n")
}

/** Runs opaque-route hypothesis; returns undefined on any failure. */
export async function hypothesizeOpaqueRoutes(provider: ModelProvider, routes: readonly OpaqueRouteInput[]): Promise<OpaqueHypothesisResult | undefined> {
  if (routes.length === 0) return { hypotheses: [] }
  try {
    return await provider.respondJson(
      { messages: [{ role: "system", content: SYSTEM }, { role: "user", content: userPrompt(routes) }], temperature: 0, maxTokens: 1500 },
