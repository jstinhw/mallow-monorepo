...
import type { AddressReport, CallChain, CompressedContract, UnboundedFrontier } from "../types/report"
...
const WRAPPER_ALIASES: Record<string, readonly { readonly member: string; readonly argShift: number }[]> = {
  transferFrom: [{ member: "safeTransferFrom", argShift: 1 }],
  approve: [{ member: "safeApprove", argShift: 1 }, { member: "forceApprove", argShift: 1 }],
}
...
function targetSpecs(tokenFn: string, argIndex: number): readonly TargetSpec[] {
  return [{ member: tokenFn, argIndex }, ...(WRAPPER_ALIASES[tokenFn] ?? []).map((a) => ({ member: a.member, argIndex: argIndex + a.argShift }))]
}
...
const EMPTY_DELTA: VisitDelta = { chains: [], nodes: [], edges: [], unbounded: [], reasons: [], compressed: [], enqueue: [] }
...
export interface RecursionResult {
  ...
  readonly perAnchor: ReadonlyMap<number, { readonly chains: readonly CallChain[]; readonly unbounded: readonly UnboundedFrontier[] }>
}

/** A caller that cannot be descended into: EOA (terminal), undetermined, or unverified. */
function visitTerminal(session: TraceSession, item: WorkItem, info: Exclude<ContractInfo, VerifiedContract>): VisitDelta {
  const { log } = session
  if (info.kind === "eoa") {
  ...
  return { ...EMPTY_DELTA, reasons: [`caller ${item.addr} is an unverified contract (source unavailable)`] }
}
...
function routeDelta(
  ...
  const head = r.constraint.kind === "caller-is-owner" ? (keyHolder ? "key-holder" : "<key-holder>") : r.constraint.kind === "unbounded" ? "any caller" : "opaque caller"
  log.note(`${position} ${head} → ${r.contract}.${r.functionName} (key-arg=${provLabel(r.fromProvenance)})`)
  ...
  if (r.constraint.kind === "caller-is-owner") {
    ...
    const b: Boundedness = { kind: "singleton", callers: keyHolder ? [keyHolder as Address] : [], guards: [] }
    return {
      ...EMPTY_DELTA,
      edges: [edge],
      nodes: [{ id: nid, codehash: info.codehash, functionName: r.functionName, touches: [], boundedness: b, verified: true }],
      chains: [{ path: [holderLabel, `${info.contractName}.${r.functionName}`, `${target.contractName}.${item.tokenFn}`], op: "read-write", boundedness: b, provenance: "verified" }],
      enqueue: keyHolder && item.depth < session.bounds.maxDepth
        ? [{ addr: keyHolder as Address, tokenFn: item.tokenFn, depth: item.depth + 1, via: `${info.contractName}.${r.functionName} → ${item.via}`, anchorIndex: item.anchorIndex, ...(item.binding ? { binding: item.binding } : {}) }]
        : [],
    }
  }
  if (r.constraint.kind === "unbounded") {
    ...
    return {
      ...
      chains: [{ path: ["<any caller>", `${info.contractName}.${r.functionName}`, `${target.contractName}.${item.tokenFn}`], op: "read-write", boundedness: b, provenance: "verified" }],
    }
  }
  const b: Boundedness = { kind: "role-set", roleVar: r.constraint.note, enumeration: "trace", guards: [] }
  return {
    ...EMPTY_DELTA,
    edges: [edge],
    nodes: [{ id: nid, codehash: info.codehash, functionName: r.functionName, touches: [], boundedness: b, verified: true }],
    reasons: [`opaque: ${info.contractName}.${r.functionName} — ${r.constraint.note} (needs dynamic/deeper analysis)`],
    chains: [{ path: ["<opaque caller>", `${info.contractName}.${r.functionName}`, `${target.contractName}.${item.tokenFn}`], op: "read-write", boundedness: b, provenance: "verified" }],
  }
}
...
function visitContract(session: TraceSession, target: VerifiedContract, anchors: readonly AnchorInstance[], item: WorkItem, info: VerifiedContract): VisitDelta {
  const { log } = session
  ...
  if (reachers.length === 0) {
    ...
    return { ...EMPTY_DELTA, reasons: [`${info.contractName} (${item.addr}) has no ${item.tokenFn} call-sites`] }
  }
  log.note(`${info.contractName} → ${reachers.length} entrypoints reach ${item.tokenFn}`)

  const slice = sliceByCallClosure(info.compiled, reachers.map((r) => r.functionName), specs.map((s) => s.member))
  const tokenSel = anchor.analysis.touchers.find((t) => t.functionName === item.tokenFn)?.selector ?? "0x"
  const routes = reachers.map((r, i) => routeDelta(session, target, anchor, item, info, slice, tokenSel, r, `[${i + 1}/${reachers.length}]`))
  return mergeDeltas([{ ...EMPTY_DELTA, compressed: [toCompressed(item.addr, info.codehash, slice)] }, ...routes])
}
...
export async function recurseCallers(
  ...
  while (queue.length > 0 && visited.size < session.bounds.maxAddresses) {
    ...
    log.sub(`visit caller ${item.addr} → ${item.via} · depth ${item.depth} · ${info.kind}${info.kind === "contract" ? "" : tagSuffix(info)}`)

    const delta = info.kind === "contract" ? visitContract(session, target, anchors, item, info) : visitTerminal(session, item, info)
    deltas.push(delta)
