...
const ALLOWANCE_ABI = [
  { type: "function", name: "allowance", inputs: [{ name: "o", type: "address" }, { name: "s", type: "address" }], outputs: [{ type: "uint256" }] },
] as const

// ── path/key extraction ──────────────────────────────────────────────────────────────────

function keyPathOf(path: readonly PathSegment[]): readonly MappingKey[] {
  return path.filter((s): s is Extract<PathSegment, { kind: "mapping-key" }> => s.kind === "mapping-key").map((s) => ({ provenance: s.candidate.origin, concrete: s.candidate.raw }))
}
...
function buildAnchor(target: VerifiedContract, item: StorageLayoutItem, valueType: string, path: readonly PathSegment[]): Anchor {
...
function isWriteAnchor(attr: AttributedSlot, source: StorageObservation["source"], writtenVars: ReadonlySet<string>): boolean {
...
function instanceFromAttributed(target: VerifiedContract, attr: AttributedSlot): AnchorInstance {
  ...
  const analysis = analyzeAnchor(target.compiled, attr.item.label, attr.item.slot, attr.item.type, concreteKeys)
  return {
    item: attr.item,
    anchor,
    analysis,
...
function staticInstance(target: VerifiedContract, item: StorageLayoutItem, keys: readonly KeyProvenance[], seed: Seed, decoded: DecodedCall): AnchorInstance | undefined {
  const concreteKeys = keys.map((k) => concretize(k, seed, decoded))
  const keyPath: readonly MappingKey[] = keys.map((provenance, i) => ({ provenance, ...(concreteKeys[i] !== undefined ? { concrete: concreteKeys[i]! } : {}) }))
  const anchor: Anchor = { codehash: target.codehash, baseSlot: item.slot, variable: item.label, keyPath, valueType: item.type }
  const analysis = analyzeAnchor(target.compiled, item.label, item.slot, item.type, concreteKeys)
  const observedSlot = keyPath.length > 0 && concreteKeys.every((k) => k !== undefined)
    ? mappingValueSlot(BigInt(item.slot), concreteKeys.map((k) => pad(k as Hex, { size: 32 })))
  ...
  return {
    ...
    concreteKeys,
    path: [],
    pathLabel: renderPath(item.label, keyPath.map((k) => ({ kind: "mapping-key", encoded: "0x", candidate: { raw: (k.concrete ?? "0x") as Hex, label: "", origin: k.provenance } }))),
    access: "static-write",
...
function syntheticAnchors(target: VerifiedContract, seed: Seed, decoded: DecodedCall): readonly AnchorInstance[] {
...
async function crossCheckAllowance(session: TraceSession, target: VerifiedContract, ai: AnchorInstance): Promise<AnchorVerification> {
  ...
  try {
    const raw = (await session.publicClient.getStorageAt({ address: target.address, slot: ai.observedSlot, blockNumber: session.blockNumber })) ?? "0x"
    const fromStorage = hexToBigInt(raw === "0x" ? "0x0" : raw)
    const { data } = await session.publicClient.call({
      to: target.address,
      data: encodeFunctionData({ abi: ALLOWANCE_ABI, functionName: "allowance", args: [owner as Address, spender as Address] }),
 ...
 */
export async function resolveSeed(session: TraceSession, seed: Seed, opts: { readonly anchorFilter?: string } = {}): Promise<ResolvedSeed> {
  const { log } = session

  log.step(`load target ${seed.to} · resolve proxy → impl → compile`)
  const target = await session.contracts(seed.to as Address)
  if (target.kind !== "contract") throw new Error(`target ${seed.to} is ${target.kind}, expected a verified contract`)
  log.sub(`${target.contractName}${target.implementation ? ` (impl ${target.implementation})` : ""} · ${target.compiled.storage.length} storage vars`)

  const call = await decodeSeedCall(seed.calldata as Hex, target.compiled.abi, log)
  log.step(`decode calldata · ${call.functionName}${call.args.length ? `(${call.args.length} args)` : ""} · via ${call.source}`)

  log.step(`observe storage · simulate at pinned block`)
  const observation = await session.observer.observe({ from: seed.from as Address, to: seed.to as Address, calldata: seed.calldata as Hex }, session.blockNumber)
  log.sub(`source ${observation.source}${observation.reverted ? " · call reverts" : ""} · ${observation.slots.length} slots touched`)

  const candidates = enumerateKeyCandidates(seed, call)
  const readLength = async (slot: Hex): Promise<Hex | undefined> => (await session.publicClient.getStorageAt({ address: seed.to as Address, slot, blockNumber: session.blockNumber })) ?? undefined

  let anchors: readonly AnchorInstance[] = []
  let unattributed: readonly SlotObservation[] = []

  if (observation.slots.length > 0) {
    const forTarget = observation.slots.filter((o) => o.address.toLowerCase() === seed.to.toLowerCase() || (target.implementation && o.address.toLowerCase() === target.implementation.toLowerCase()))
    const result = await attributeSlots(target.compiled.storage, target.compiled.storageTypes, forTarget, candidates, readLength)
    unattributed = result.unattributed
    const writtenVars = new Set(analyzeWrites(target.compiled, call.functionName === "unknown" ? "" : call.functionName, call.args.length).map((w) => w.varName))
    const writeSlots = result.attributed.filter((a) => isWriteAnchor(a, observation.source, writtenVars))
    const grouped = new Map<string, AttributedSlot>()
    for (const a of writeSlots) {
      const key = `${a.item.label}|${keyPathOf(a.path).map((k) => k.concrete).join(",")}|${elementPathOf(a.path).join(".")}`
  ...
  if (anchors.length === 0) {
    anchors = await Promise.all(syntheticAnchors(target, seed, call).map(async (ai) => ({ ...ai, verification: await crossCheckAllowance(session, target, ai) })))
  }
  ...
  for (const a of anchors) log.sub(`${a.pathLabel} @ slot ${a.item.slot} · ${a.analysis.touchers.length} touchers · ${a.verification}`)
