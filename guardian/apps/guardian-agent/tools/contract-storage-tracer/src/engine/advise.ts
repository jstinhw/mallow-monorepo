...
function opaqueRoutes(recursion: RecursionResult): readonly OpaqueRouteInput[] {
  ...
  for (const reason of recursion.reasons) {
    ...
    if (!m) continue;
    const compressed = recursion.compressed.find(
      (c) => c.sourceSlice.includes(`${m[2]}(`) || c.functions.includes(m[2]!),
    );
    out.push({
      contract: m[1]!,
      functionName: m[2]!,
      note: m[3]!,
      sourceSlice: compressed?.sourceSlice ?? "",
    });
  }
  return out;
}
...
export async function applyAdvisory(
  ...
  if (anchorVars.size > 1) {
    const sourceSlice = sliceByStorageCoupling(resolved.target.compiled, [
      ...anchorVars,
    ]).sourceSlice;
    const result = await triageAnchors(llm.provider, {
      ...
      anchors: resolved.anchors.map((a) => ({
        variable: a.item.label,
        type: a.anchor.valueType,
        path: a.pathLabel,
        access: a.access,
      })),
      sourceSlice,
