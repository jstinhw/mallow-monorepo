 ...
 */
export async function executeTrace(seed: Seed, options: TraceOptions = {}, log?: StepLogger): Promise<TraceReport> {
  const config = loadConfig()
  const logger = log ?? createStepLogger(config.traceLog)
  const { chain, rpcUrl } = getSupportedChain(seed.chainId)
  const publicClient = createPublicClient({ chain, transport: http(rpcUrl) })
  const blockNumber = seed.block === undefined ? await publicClient.getBlockNumber() : BigInt(seed.block)
  ...
  const tokenSides = resolved.anchors.map((ai, i) => classifyTouchers(session, resolved.target, ai, i))
