...
function bindingOf(keys: readonly KeyProvenance[]): { readonly keyIndex: number; readonly argIndex: number } | undefined {
 ...
 */
export function classifyTouchers(session: TraceSession, target: VerifiedContract, ai: AnchorInstance, anchorIndex: number): TokenClassification {
  ...
  for (const t of analysis.touchers) {
    ...
    for (const caller of singletonCallers(t.boundedness)) {
      chains.push({ path: [caller, `${target.contractName}.${t.functionName}`], op: t.op, boundedness: t.boundedness, provenance: "verified" })
      ...
      frontier.set(`${caller.toLowerCase()}:${t.functionName}`, { addr: caller, tokenFn: t.functionName, via: `${target.contractName}.${t.functionName}`, anchorIndex, ...(binding ? { binding } : {}) })
    }
    if (t.boundedness.kind === "unbounded") {
      unbounded.push({ node: `${target.contractName}.${t.functionName}`, reason: t.boundedness.reason })
      reasons.push(`unbounded: ${target.contractName}.${t.functionName} (${t.op})`)
      chains.push({ path: ["<any caller>", `${target.contractName}.${t.functionName}`], op: t.op, boundedness: t.boundedness, provenance: "verified" })
    }
    if (t.boundedness.kind === "role-set") reasons.push(`role-set: ${target.contractName}.${t.functionName} via ${t.boundedness.roleVar} (enumerate off-chain)`)
  }
