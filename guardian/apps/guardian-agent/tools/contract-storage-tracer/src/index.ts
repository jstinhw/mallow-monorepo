...
async function main(): Promise<void> {
  ...
  const seed = parseSeed({ chainId: args.chainId, from: args.from, to: args.to, calldata: args.calldata, block: args.block })

  const report = await executeTrace(seed, { noLlm: args.noLlm, ...(args.anchor ? { anchorFilter: args.anchor } : {}) })
