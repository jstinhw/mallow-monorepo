...
export function resolveExpr(expr: AstNode | undefined, argProv: ArgProv): KeyProvenance {
  ...
  if (expr.nodeType === "MemberAccess") {
    ...
    if (base?.nodeType === "Identifier" && base.name === "msg" && expr.memberName === "sender") return { kind: "msg.sender" }
    if (base?.nodeType === "Identifier" && base.name === "tx" && expr.memberName === "origin") return { kind: "tx.origin" }
  ...
  if (expr.nodeType === "FunctionCall") {
    ...
    if (callee?.nodeType === "Identifier" && callee.name === "ecrecover") return { kind: "derived", note: "ecrecover" }
...
function collect(index: AstIndex, fn: AstNode, argProv: ArgProv, anchorVarId: number, depth: number, seen: Set<number>, targetMembers: ReadonlySet<string> = new Set()): Collected {
  ...
  for (const mod of (fn.modifiers as AstNode[] | undefined) ?? []) {
    ...
    if (name) guards.push({ source: "modifier", expression: String(name), bindsCaller: /^only/i.test(String(name)), provenance: "verified" })
  }
  ...
  const visit = (node: unknown, write: Access | null): void => {
    ...
    if (n.nodeType === "MemberAccess") {
      ...
      if (chain && chain.varId === anchorVarId) {
        accesses.push({ op: write ?? "read", keys: chain.keys.map((k) => resolveExpr(k, argProv)) })
        return
      }
    }
    // Bare reference to a scalar anchor (e.g. `paused = true`, or reading `config`).
    if (n.nodeType === "Identifier" && (n.referencedDeclaration as number | undefined) === anchorVarId) {
      accesses.push({ op: write ?? "read", keys: [] })
      return
    }
    ...
    if (n.nodeType === "FunctionCall") {
      ...
      if (callee?.nodeType === "Identifier") {
        ...
        if (name === "require" || name === "assert") {
          guards.push({ source: "require", expression: describeRequire(n), bindsCaller: bindsSender(n), provenance: "verified" })
        }
        const args = (n.arguments as AstNode[] | undefined) ?? []
        const target = resolveCallee(index, name, args.length, callee.referencedDeclaration as number | undefined, anchorVarId)
        if (target?.nodeType === "FunctionDefinition" && depth < MAX_DEPTH && typeof target.id === "number" && !seen.has(target.id)) {
          ...
          params.forEach((p, i) => { if (typeof p.id === "number") childProv.set(p.id, resolveExpr(args[i], argProv)) })
          const child = collect(index, target, childProv, anchorVarId, depth + 1, new Set([...seen, target.id]), targetMembers)
      ...
      if (callee?.nodeType === "MemberAccess") {
        ...
        if (targetMembers.has(member)) {
          const args = ((n.arguments as AstNode[] | undefined) ?? []).map((a) => resolveExpr(a, argProv))
...
const exprName = (e: AstNode | undefined): string => {
  if (!e) return "?"
  if (e.nodeType === "MemberAccess") return `${exprName(e.expression as AstNode)}.${String(e.memberName)}`
...
function bindsSender(call: AstNode): boolean {
  ...
  return [arg.leftExpression, arg.rightExpression].some((s) => exprName(s as AstNode).includes("msg.sender"))
}

/** Combines key provenance + concrete anchor keys + guards into a boundedness verdict. */
function solveBoundedness(keys: readonly KeyProvenance[], concrete: readonly (string | undefined)[], guards: readonly Guard[], hasEcrecover: boolean): Boundedness {
  for (let i = 0; i < keys.length; i++) {
    const c = concrete[i]
    if (keys[i]?.kind === "msg.sender" && c) return { kind: "singleton", callers: [c as `0x${string}`], guards }
  }
  ...
  if (hasEcrecover) {
    return { kind: "unbounded", reason: "permissionless entrypoint; effect gated by owner signature (ecrecover), caller identity unconstrained", guards }
  }
  return { kind: "unbounded", reason: "no msg.sender-keyed dimension or guard binds the caller to this concrete entry", guards }
}
...
function representativeKeys(accesses: readonly Access2[]): readonly KeyProvenance[] {
  const ranked = [...accesses].sort((a, b) => Number(b.op !== "read") - Number(a.op !== "read") || b.keys.length - a.keys.length)
...
function findStateVarId(index: AstIndex, name: string): number | undefined {
  ...
  for (const node of index.byId.values()) {
    if (node.nodeType === "VariableDeclaration" && node.stateVariable === true && node.name === name && typeof node.id === "number") id = node.id
  }
  return id
}

export function resolveEntry(index: AstIndex, name: string, arity: number): AstNode | undefined {
  const impls = (index.implemented.get(name) ?? []).filter((f) => functionParams(f).length === arity)
...
export function stateVarNames(index: AstIndex): Map<number, string> {
  ...
  for (const node of index.byId.values()) {
    if (node.nodeType === "VariableDeclaration" && node.stateVariable === true && typeof node.id === "number" && node.name) {
 ...
 */
export function analyzeAnchor(compiled: Compiled, anchorVar: string, baseSlot: string, anchorType: string, concreteKeys: readonly (string | undefined)[]): AnchorAnalysis {
  ...
  if (anchorVarId !== undefined) {
    for (const item of abi) {
      ...
      functionParams(fn).forEach((p, i) => { if (typeof p.id === "number") argProv.set(p.id, { kind: "arg", index: i, ...(p.name ? { name: String(p.name) } : {}) }) })
...
function fromOf(t: ExternalTargetCall, argIndexByMember: ReadonlyMap<string, number>): KeyProvenance {
...
function constraintOf(from: KeyProvenance): SpenderConstraint {
  if (from.kind === "arg") return { kind: "unbounded", reason: `the anchor-key argument is caller-supplied ('${from.name ?? `#${from.index}`}'); any address can direct this call at the key-holder's slot` }
  if (from.kind === "msg.sender") return { kind: "caller-is-owner" }
  if (from.kind === "derived") return { kind: "opaque", note: `anchor-key argument derives from ${from.note}` }
 ...
 */
export function analyzeCallers(compiled: Compiled, targets: readonly TargetSpec[]): readonly CallerRoute[] {
  const index = indexAst(compiled.asts)
  ...
  for (const item of abi) {
    ...
    const target = [...c.externalTargets].sort((a, b) => severity(fromOf(a, argIndexByMember)) - severity(fromOf(b, argIndexByMember)))[0]!
    const from = fromOf(target, argIndexByMember)
    const contract = (typeof fn.id === "number" ? ownerContract.get(fn.id) : undefined) ?? compiled.contractName
    out.set(item.name, { functionName: item.name, contract, selector: toFunctionSelector(item), member: target.member, fromProvenance: from, constraint: constraintOf(from) })
  }
  return [...out.values()]
}
