...
function exprShort(e: AstNode | undefined): string {
  ...
  if (e.nodeType === "MemberAccess") return `${exprShort(e.expression as AstNode)}.${String(e.memberName)}`
...
function buildModel(compiled: Compiled): Model {
  ...
  for (const node of index.byId.values()) {
    ...
    if (node.nodeType === "FunctionDefinition" && typeof node.id === "number") fnById.set(node.id, node)
    ...
    walk(fn.body, (n) => {
      ...
      if (n.nodeType === "FunctionCall") {
        ...
        if (callee?.nodeType === "Identifier") {
          ...
          const target = resolveCallee(index, String(callee.name), arity, callee.referencedDeclaration as number | undefined, -1)
          if (target?.nodeType === "FunctionDefinition" && typeof target.id === "number") called.add(target.id)
        }
        if (callee?.nodeType === "MemberAccess") {
          sites.push({ member: String(callee.memberName), args: ((n.arguments as AstNode[] | undefined) ?? []).map(exprShort) })
        }
      }
    })
...
function fnIdsByName(model: Model, names: readonly string[]): Set<number> {
  const ids = new Set<number>()
  for (const name of names) for (const f of model.index.implemented.get(name) ?? []) if (typeof f.id === "number") ids.add(f.id)
  return ids
}
 ...
 */
export function sliceByStorageCoupling(compiled: Compiled, varNames: readonly string[]): CompressedSlice {
  ...
  for (let changed = true; changed; ) {
    ...
    for (const [id] of model.fnById) {
      ...
      if ([...trans(id)].some((v) => relevantVars.has(v))) { relevantFns.add(id); changed = true }
    }
    for (const id of relevantFns) {
      ...
      for (const v of direct) if (!relevantVars.has(v)) { relevantVars.add(v); changed = true }
    }
  }
...
function transitiveCallees(model: Model, seed: number): Set<number> {
  ...
  while (queue.length > 0) {
    for (const callee of model.callees.get(queue.shift()!) ?? []) {
      if (callee !== seed && !seen.has(callee)) { seen.add(callee); queue.push(callee) }
    }
  }
  return seen
}
...
function callTree(model: Model, id: number, label: (id: number) => string, targets: ReadonlySet<string>, path: ReadonlySet<number>): CallTreeNode {
  ...
  const external = (model.memberCalls.get(id) ?? [])
    ...
    .map((site) => ({ label: `${site.member}(${site.args.join(", ")})`, children: [], external: true }))
 ...
 */
export function sliceByCallClosure(compiled: Compiled, fnNames: readonly string[], targetMembers: readonly string[] = []): CallClosureSlice {
  const model = buildModel(compiled)
  ...
  for (const id of relevantFns) for (const v of model.directTouch.get(id) ?? []) relevantVars.add(v)
  ...
  const closures = perSeed
    .map((p) => ({ entry: label(p.seed), tree: callTree(model, p.seed, label, targets, new Set([p.seed])) }))
...
function extract(node: AstNode, files: Map<number, string>): { text: string; file: number; start: number } | undefined {
  const src = node.src as string | undefined
  if (!src) return undefined
  const [start, len, file] = src.split(":").map(Number) as [number, number, number]
  const content = files.get(file)
  if (content === undefined) return undefined
  return { text: Buffer.from(content, "utf8").subarray(start, start + len).toString("utf8"), file, start }
}

const fnLabel = (n: AstNode): string => String(n.name) || String(n.kind ?? "fn")

function render(compiled: Compiled, model: Model, vars: Set<number>, fns: Set<number>): CompressedSlice {
  const files = fileContents(compiled)
  const order = (a: { file: number; start: number }, b: { file: number; start: number }) => a.file - b.file || a.start - b.start
  ...
  const fnDefs = [...fns]
    .flatMap((id) => {
      ...
      return e && node ? [{ ...e, contract: model.ownerContract.get(id) ?? "", name: fnLabel(node) }] : []
    })
    .filter((f) => (seenF.has(f.text) ? false : (seenF.add(f.text), true)))
    .sort(order)

  const varNames = [...new Set([...vars].flatMap((id) => { const v = model.varById.get(id); return v ? [String(v.name)] : [] }))]
  const fnNames = [...new Set([...fns].flatMap((id) => { const n = model.fnById.get(id); return n?.body ? [fnLabel(n)] : [] }))]
