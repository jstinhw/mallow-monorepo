...
function rawOf(value: unknown): Hex | undefined {
  if (typeof value === "string" && /^0x[0-9a-fA-F]*$/.test(value)) return value.toLowerCase() as Hex
  ...
  if (typeof value === "number" && Number.isInteger(value)) return toHex(BigInt(value < 0 ? BigInt.asUintN(256, BigInt(value)) : value))
...
function flattenArg(value: unknown, type: AbiParameter | undefined, index: number, label: string): KeyCandidate[] {
  ...
  const components = type && "components" in type ? (type.components as readonly AbiParameter[] | undefined) : undefined
  if (components && value && typeof value === "object" && !Array.isArray(value)) {
    for (const comp of components) {
      const field = (value as Record<string, unknown>)[comp.name ?? ""]
      if (field !== undefined) out.push(...flattenArg(field, comp, index, `${label}.${comp.name ?? "?"}`))
    }
  }
  return out
}
...
export function enumerateKeyCandidates(seed: Seed, decoded: DecodedCall): readonly KeyCandidate[] {
  ...
  const out: KeyCandidate[] = [
    ...
    { raw: seed.to.toLowerCase() as Hex, label: "seed.to", origin: { kind: "derived", note: "seed.to" } },
