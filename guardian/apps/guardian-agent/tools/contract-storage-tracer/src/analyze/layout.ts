 ...
 */
export function slotSpan(item: Pick<StorageLayoutItem, "slot" | "type">, types: Record<string, SolcTypeInfo>): { first: bigint; last: bigint } {
...
function isDynamicBytesType(typeId: string): boolean {
  return typeId === "t_string_storage" || typeId === "t_bytes_storage" || typeId.startsWith("t_string") || typeId === "t_bytes"
}
...
export function packedMembersInSlot(
  ...
  types: Record<string, SolcTypeInfo>,
  slot: bigint,
): readonly { readonly item: StorageLayoutItem; readonly byteLo: number; readonly byteHi: number }[] {
  return items
