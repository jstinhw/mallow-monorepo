...
export function subtreeTouches(node: unknown, varId: number): boolean {
  ...
  walk(node, (n) => {
    ...
    else if (n.nodeType === "Identifier" && (n.referencedDeclaration as number | undefined) === varId) found = true
  })
  return found
}
...
export function inheritedContractNames(index: AstIndex, contractName: string): readonly string[] {
  ...
  return baseIds
    ...
    .filter((n): n is AstNode => n !== undefined && n.name !== contractName && n.contractKind !== "interface")
    .map((n) => String(n.name))
}
