...
function loadCompiler(version: string): Promise<SolcModule> {
  ...
  const loading = new Promise<SolcModule>((resolve, reject) => {
    ;(solcWrapper as { loadRemoteVersion(v: string, cb: (e: Error | null, s: SolcModule) => void): void })
...
interface SolcOutput {
  ...
  readonly contracts?: Record<
    ...
    Record<
      string,
      {
        readonly storageLayout?: { storage: StorageLayoutItem[]; types?: Record<string, SolcTypeInfo> | null }
...
export async function compile(source: VerifiedSource): Promise<Compiled> {
  ...
  const input = {
    language: "Solidity",
    sources: Object.fromEntries(Object.entries(source.sources).map(([p, content]) => [p, { content }])),
    settings: {
  ...
  if (fatal.length > 0) {
    throw new Error(`solc ${source.compilerVersion} failed:\n${fatal.slice(0, 3).map((e) => e.formattedMessage).join("\n")}`)
  }
