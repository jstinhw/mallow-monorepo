...
function delegatedEoaTarget(code: Hex): Address | undefined {
  return code.length === 48 && code.slice(0, 8) === EIP7702_PREFIX
...
export async function loadContract(
  ...
  if (code !== "0x") {
    ...
    if (delegate) {
      return {
        kind: "eoa",
        address,
        enrichment: await enrich(publicClient, blockNumber, address, tagger),
        delegation: delegate,
      };
    }
  ...
  return enrichment.sentTxs
    ? { kind: "eoa", address, enrichment }
    : { kind: "undetermined", address, enrichment };
}
