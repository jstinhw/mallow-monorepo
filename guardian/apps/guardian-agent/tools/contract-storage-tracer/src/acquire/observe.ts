...
import { accessListSchema, prestateDiffSchema, prestateFlatSchema, type AccessListResult, type PrestateDiff, type PrestateFlat } from "../schemas/rpc"
...
export function touchesFromPrestateFlat(flat: PrestateFlat, writeKeys: ReadonlySet<string>): readonly SlotObservation[] {
  const out: SlotObservation[] = []
  for (const [address, account] of Object.entries(flat)) {
    for (const [slot, value] of Object.entries(account.storage ?? {})) {
      ...
      out.push({ address: address as Address, slot: slot as Hex, access: "touch", pre: value as Hex })
    }
  }
  return out
}
...
export function touchesFromAccessList(result: AccessListResult): readonly SlotObservation[] {
  return result.accessList.flatMap((entry) =>
    entry.storageKeys.map((slot) => ({ address: entry.address as Address, slot: slot as Hex, access: "touch" as const })),
  )
}

const writeKey = (o: SlotObservation): string => `${o.address.toLowerCase()}:${o.slot.toLowerCase()}`
...
function withTimeout<T>(promise: Promise<T>, ms: number): Promise<T> {
  return Promise.race([
    promise,
    new Promise<T>((_, reject) => setTimeout(() => reject(new Error(`RPC call timed out after ${ms}ms`)), ms).unref?.()),
  ])
}
...
async function detectRevert(publicClient: PublicClient, seed: ObserveSeed, blockNumber: bigint): Promise<boolean> {
  try {
...
export function createObserver(publicClient: PublicClient): StorageObserver {
  ...
  async function tryPrestateDiff(seed: ObserveSeed, blockNumber: bigint): Promise<readonly SlotObservation[] | undefined> {
    if (debugSupported === false) return undefined
    const callObj = { from: seed.from, to: seed.to, data: seed.calldata }
    try {
      const raw = await withTimeout(
        request({ method: "debug_traceCall", params: [callObj, toHex(blockNumber), { tracer: "prestateTracer", tracerConfig: { diffMode: true } }] }),
        TRACE_TIMEOUT_MS,
      )
      debugSupported = true
  ...
  async function tryPrestateReads(callObj: object, blockNumber: bigint, writeKeys: ReadonlySet<string>): Promise<readonly SlotObservation[]> {
    try {
      const raw = await withTimeout(request({ method: "debug_traceCall", params: [callObj, toHex(blockNumber), { tracer: "prestateTracer" }] }), TRACE_TIMEOUT_MS)
  ...
  async function tryAccessList(seed: ObserveSeed, blockNumber: bigint): Promise<readonly SlotObservation[] | undefined> {
    if (accessListSupported === false) return undefined
    try {
      const raw = await withTimeout(request({ method: "eth_createAccessList", params: [{ from: seed.from, to: seed.to, data: seed.calldata }, toHex(blockNumber)] }), TRACE_TIMEOUT_MS)
