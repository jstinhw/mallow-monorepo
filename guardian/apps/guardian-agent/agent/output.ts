...
const DECISION_LABEL = { sign: "SAFE TO SIGN", review: "REVIEW BEFORE SIGNING", reject: "DO NOT SIGN" } as const
...
export function formatVerdictReport(report: AgentReport): string {
  ...
  lines.push(
    "## Evidence",
    ...evidence.toolCalls.map((tc) => `- round ${tc.round} \`${tc.tool}\` ${tc.ok ? "ok" : "error"} (${tc.durationMs}ms)`),
    "",
...
export function writeAgentOutputs(report: AgentReport, outDir: string): readonly string[] {
  ...
  const files = [
    ...
    ["contract-storage-tracer.json", `${JSON.stringify(report.tools.contractStorageTracer, null, 2)}\n`],
