...
export type ToolResult = { readonly ok: true; readonly result: unknown } | { readonly ok: false; readonly error: string }
...
const getCallChains: AgentTool = {
  ...
  execute(args, ctx) {
    ...
    if (variable && anchors.length === 0) {
      return { error: `no anchor matches '${variable}'; anchors: ${ctx.trace.anchors.map((a) => a.path).join(", ")}` }
    }
    const chains = (variable ? anchors.flatMap((a) => a.chains) : ctx.trace.chains).map(compactChain)
    return {
      chains: chains.slice(0, MAX_CHAINS),
      ...(chains.length > MAX_CHAINS ? { truncated: `${chains.length - MAX_CHAINS} more omitted` } : {}),
    }
  },
}
...
const readContractSlice: AgentTool = {
  ...
  execute(args, ctx) {
    ...
    if (!contract) {
      return { error: `no compressed contract at ${address}; available: ${ctx.trace.compressedContracts.map((c) => c.address).join(", ")}` }
    }
    return {
...
const inspectAddress: AgentTool = {
  ...
  execute(args, ctx) {
    ...
    return entry ?? { error: `address ${address} not in the trace's address book — it does not appear on any route` }
  },
}
...
export function executeAgentTool(name: string, args: unknown, ctx: AgentToolContext): ToolResult {
  ...
  if (!tool) return { ok: false, error: `unknown tool '${name}'; available: ${AGENT_TOOLS.map((t) => t.name).join(", ")}` }
  const parsed = tool.argsSchema.safeParse(args ?? {})
  if (!parsed.success) return { ok: false, error: `invalid args for ${name}: ${parsed.error.issues.map((i) => i.message).join("; ")}` }
  try {
...
export function traceFacts(trace: TraceReport): Record<string, unknown> {
  ...
  for (const c of trace.chains) byBoundedness.set(c.boundedness.kind, (byBoundedness.get(c.boundedness.kind) ?? 0) + 1)
  return {
    ...
    writtenVariables: trace.anchors.map((a) => ({
      ...
      ...(a.triage ? { triage: { label: a.triage.label, securityRelevance: a.triage.securityRelevance, note: "hypothesis" } } : {}),
    })),
    ...
    ...(trace.hypotheses?.length
      ? { routeHypotheses: trace.hypotheses.map((h) => `[${h.confidence}] ${h.route}: ${h.controller} — ${h.explanation}`) }
      : {}),
  }
}
