...
export const SUPPORTED_CHAINS: Record<number, SupportedChain> = {
  [mainnet.id]: {
    chain: mainnet,
    get rpcUrl() {
      return alchemyRpcUrl("eth-mainnet");
    },
  },
  [arbitrum.id]: {
    chain: arbitrum,
    get rpcUrl() {
      return alchemyRpcUrl("arb-mainnet");
    },
  },
  [bsc.id]: {
    chain: bsc,
    get rpcUrl() {
      return alchemyRpcUrl("bnb-mainnet");
    },
  },
  [base.id]: {
    chain: base,
    get rpcUrl() {
      return alchemyRpcUrl("base-mainnet");
    },
  },
  [optimism.id]: {
    chain: optimism,
    get rpcUrl() {
      return alchemyRpcUrl("opt-mainnet");
    },
  },
  [polygon.id]: {
    chain: polygon,
    get rpcUrl() {
      return alchemyRpcUrl("polygon-mainnet");
    },
  },
};
...
export const chainIdSchema = z.coerce
  ...
  .int()
  .refine(
    (id): id is number => supportedChainIdSet.has(id),
    (id) => ({
