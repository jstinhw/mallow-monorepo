...
export function assembleReport(
    ...
    anchorLabels.length > 0
      ? [
          toCompressed(
            target.address,
            target.codehash,
            sliceByStorageCoupling(target.compiled, anchorLabels),
          ),
        ]
      : [];
  ...
  const addresses = new Map<string, AddressReport>([
    [target.address.toLowerCase(), toAddressReport(target)],
    ...recursion.addresses,
  ]);
  const bookTags = new Map<string, string>([
    [target.address.toLowerCase(), bookTag(target)],
    ...recursion.bookTags,
  ]);
  ...
  const orderedAnchors = advisory.triage
    ? [...anchorReports].sort((a, b) => (a.triage?.rank ?? 1e9) - (b.triage?.rank ?? 1e9))
    : anchorReports;

  log.step(`address book · ${addresses.size} addresses`);
  for (const a of addresses.values())
    log.sub(`${addressBookLine(a)}${bookTags.get(a.address.toLowerCase()) ?? ""}`);
  ...
  if (resolved.unattributed.length > 0)
    reasons.push(
      `${resolved.unattributed.length} observed slot(s) could not be attributed to a layout variable`,
    );
  if (observation.reverted)
    reasons.push("the seed call reverts at the pinned block; anchors are static hypotheses");
  if (instances.length === 0)
    reasons.push(
      "the seed call writes no attributable storage; nothing to trace (reads are context only)",
    );
  const coverage: Coverage =
    reasons.length === 0 ? { kind: "complete" } : { kind: "partial", reasons };
  log.step(
    `done · ${instances.length} anchors · ${chains.length} chains · ${compressed.length} contracts · ${unbounded.length} unbounded · coverage ${coverage.kind}`,
  );

  const block = Number(session.blockNumber);
  const graph: Sarg = {
    anchors: instances.map((a) => a.anchor),
    nodes: [...tokenSides.flatMap((t) => t.nodes), ...recursion.nodes],
    edges: recursion.edges,
  };
  const anchorPhrase =
    instances.length === 1 ? instances[0]!.pathLabel : `${instances.length} anchors`;
  const seedSummary =
  ...
  return {
    ...
    observation: {
      ...
      unattributed: observation.slots.length
        ? resolved.unattributed.map((o) => ({
            slot: o.slot,
            ...(o.pre ? { pre: o.pre } : {}),
            ...(o.post ? { post: o.post } : {}),
          }))
    ...
    coverage,
    graph,
    ...(advisory.hypotheses && advisory.hypotheses.length
      ? { hypotheses: advisory.hypotheses }
      : {}),
  };
}
