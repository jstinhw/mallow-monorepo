...
export function toAddressReport(info: ContractInfo): AddressReport {
  if (info.kind === "contract")
    return { address: info.address, classification: "contract", note: info.contractName };
  ...
  if (info.kind === "eoa") {
    return {
      ...
      classification: "eoa",
      ...(info.delegation
        ? { delegation: info.delegation, note: `EIP-7702 delegated EOA → ${info.delegation}` }
        : {}),
    };
  }
  if (info.kind === "undetermined") {
    return {
      ...
      note: "no code and no sent txs — EOA (unused) or an undeployed/counterfactual contract",
    };
  }
...
export function toCompressed(
  address: Address,
  codehash: string,
  slice: CompressedSlice,
): CompressedContract {
  return {
    address,
    codehash,
