...
function describeInplace(
  ...
  if (info?.members) {
    ...
    if (here.length === 1) {
      ...
      return describeInplace(
        ...
        topItem,
        obs,
        [...prefix, { kind: "struct-member", member: m.label }],
        info.members,
      );
    }
    ...
    return here.map((m) => ({
      ...
      path: [
        ...prefix,
        { kind: "struct-member" as const, member: m.label },
        packedSegment(m, types, obs),
      ],
    }));
  }
  ...
  if (info?.encoding === "inplace" && info.base) {
    ...
    return describeInplace(
      ...
      topItem,
      obs,
      [...prefix, { kind: "array-index", index: idx.index.toString(), dynamic: false }],
      siblings,
    );
  }
  ...
  if (coPacked.length > 1 && prefix.length === 0) {
    return coPacked.map((s) => ({
      observed: obs,
      item: s,
      valueType: s.type,
      path: [packedSegment(s, types, obs)],
    }));
  }
...
async function matchValueAt(
  ...
  if (info?.encoding === "dynamic_array" && info.base) {
    ...
    for (const [s, obs] of observedByBig) {
      ...
      const attrs = describeInplace(
        ...
        topItem,
        obs,
        [...prefix, { kind: "array-index", index: i.toString(), dynamic: true }],
        siblings,
      );
      if (attrs.length > 0) {
        matched.add(s);
  ...
  if (info?.encoding === "bytes") {
    ...
    if (observedByBig.has(base) && !matched.has(base)) {
      matched.add(base);
      out.push({
        observed: observedByBig.get(base)!,
        item: topItem,
        valueType: typeId,
        path: [...prefix],
      });
...
async function arrayLength(
  base: bigint,
  lenObs: SlotObservation | undefined,
  readLength: ((slot: Hex) => Promise<Hex | undefined>) | undefined,
...
async function matchMapping(
  ...
  while (worklist.length > 0) {
    ...
    for (const candidate of candidates) {
      ...
      const path: readonly PathSegment[] = [
        ...frame.path,
        { kind: "mapping-key", encoded, candidate },
      ];
